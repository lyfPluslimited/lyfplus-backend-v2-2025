...
class SelcomPaymentController extends Controller
{
    ...
    public function processOtherMobilePayment(Request $request){
        ...
        if($response['result'] == 'SUCCESS'){
            ...
            $selcomOrder->paymentToken = $response['data'][0]['payment_token'];
            $selcomOrder->save();
            ...
            $ref->set([
               ...
               "payerPhone" => $userPhone,
                ...
                "paymentToken" => $response['data'][0]['payment_token']
                ]);
    ...
    public function resendUSSDOrder(Request $request){
        $orderID = $request->orderID;

        $data = SelcomModel::where('order_id', $orderID)->first(); 

        $checkIfUserExists = User::where('userID', $data->user_id)->exists();

        if(!$checkIfUserExists){
            return response()->json('User does not exist', 400);
        }

        $user = User::where('userID', $data->user_id)->first();

        $userPhone = ltrim($data->phonenumber_used, '+');
    ...
    public function deleteOrder(Request $request){
        ...
        $order = SelcomModel::where('order_id', $orderId)->first();

        $ref = $database->getReference('Payments/'.$order->user_id.'/'.$orderId);
        ...
        return response()->json('Failed to delete order', 400);
    }
