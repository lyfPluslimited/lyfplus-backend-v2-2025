...
class ForumController extends Controller
{
    ...
    public function getAllForums(){
        $posts = DB::table('userpost AS p')->join('careusers AS u', 'u.userID', '=', 'p.userID')
                    ...
                    ->select('p.userPostID as id','p.title as title','p.description as description',
                    'p.slug as slug','p.userPrivacy as privacy','p.post_image as image', 
    ...
    public function forumComments($id){
        // $comments = Comment::where('userPostID', $id)->with('user')->get();
        ...
        $comments = DB::table('userpostcomment AS c')
                        ...
                        ->select(
                            ...
                            'u.userRole AS role',
                            'u.userID AS userID',
                            DB::raw("CONCAT(UNIX_TIMESTAMP(DATE(c.timePosted)), '000000') as timePosted"),
                            'c.postCommentID',
                            'c.userComment'
                        )
                        ->get();
    ...
    public function getForumByTopic(Request $request)
    {
       $validator = Validator::make($request->all(), [
           'id'=>'required',
       ]);
